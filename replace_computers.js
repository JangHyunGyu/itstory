...
const replacements = [
    [
        /<p class="timeline-group__summary">매일매일 끝없는 숫자 계산에 지친 수학자 아저씨들이 "기계가 대신 계산해 주면 얼마나 좋을까\?"라는 고민을 했죠\. 그래서 톱니바퀴를 뚝딱뚝딱 조립하기 시작했답니다\.<\/p>/g,
    ...
    [
        /<p class="timeline-group__summary">사람이 일일이 말해주지 않아도, 구멍 뚫린 카드와 기호만 있으면 기계가 스스로 움직일 수 있다는 신기한 생각이 퐁퐁 솟아나기 시작했어요\.<\/p>/g,
        '<p class="timeline-group__summary">사람이 일일이 개입하지 않아도, 구멍 뚫린 카드와 기호만 있으면 기계가 정해진 절차에 따라 스스로 작동할 수 있다는 혁신적인 아이디어가 등장했습니다.</p>'
    ],
    [
        /<p class="timeline-group__summary">"계산한다는 건 도대체 뭘까\?"라는 깊은 고민과 함께, 전기로 움직이는 신기한 회로 실험이 나란히 시작된 재미있는 시기였답니다\.<\/p>/g,
        '<p class="timeline-group__summary">"계산한다는 행위의 본질은 무엇일까?"라는 수학적 고민과 함께, 전기를 이용한 논리 회로 실험이 본격적으로 시작된 시기입니다.</p>'
    ],
    [
        /<p class="timeline-group__summary">반짝이는 진공관 수천 개를 모아서 눈 깜짝할 사이에 정답을 척척 내놓는 마법 같은 전자식 컴퓨터가 드디어 세상에 짠! 하고 나타났어요\.<\/p>/g,
        '<p class="timeline-group__summary">수천 개의 진공관을 연결해 인간이 며칠 걸릴 계산을 단 몇 초 만에 해결하는 최초의 범용 전자식 컴퓨터가 세상에 등장했습니다.</p>'
    ],
    [
        /<p class="timeline-group__summary">회사와 나라에서 컴퓨터를 일꾼으로 쓰기 시작했어요\. 뜨거운 진공관 대신 작고 튼튼한 트랜지스터가 등장해서 컴퓨터가 훨씬 작고 똑똑해졌답니다\.<\/p>/g,
        '<p class="timeline-group__summary">기업과 정부 기관이 컴퓨터를 업무에 도입하기 시작했습니다. 크고 뜨거운 진공관 대신 작고 효율적인 트랜지스터가 발명되면서 컴퓨터는 비약적으로 발전했습니다.</p>'
    ],
    [
        /<p class="timeline-group__summary">"컴퓨터를 바꿔도 쓰던 프로그램은 그대로 쓰고 싶어요!"라는 사람들의 소원이 모여서, 어떤 기계든 척척 맞춰주는 똑똑한 운영체제가 태어났어요\.<\/p>/g,
        '<p class="timeline-group__summary">"하드웨어를 교체해도 기존 소프트웨어를 그대로 사용할 수 없을까?"라는 산업계의 요구에 부응하여, 호환성을 갖춘 시스템과 현대적인 운영체제가 탄생했습니다.</p>'
    ],
    [
        /<p class="timeline-group__summary">컴퓨터의 뇌를 작은 칩 하나에 쏙 넣는 마법이 일어났어요! 덕분에 "우리 집에도 컴퓨터를 놓고 싶어!"라는 꿈이 진짜로 이루어졌답니다\.<\/p>/g,
        '<p class="timeline-group__summary">컴퓨터의 핵심 연산 장치를 작은 칩 하나에 집적한 마이크로프로세서가 발명되었습니다. 덕분에 개인용 컴퓨터(PC) 시대의 막이 올랐습니다.</p>'
    ],
    [
        /<p class="timeline-group__summary">누구나 쉽게 쓸 수 있는 개인용 컴퓨터가 쏟아져 나왔어요\. 그리고 전 세계의 문서를 거미줄처럼 연결하는 신기한 마법도 우리 생활 속으로 쏙 들어왔죠\.<\/p>/g,
        '<p class="timeline-group__summary">표준화된 개인용 컴퓨터가 대중화되며 사무실과 가정에 보급되었습니다. 또한 전 세계의 정보를 연결하는 월드 와이드 웹(WWW)의 개념이 처음 제안되었습니다.</p>'
    ],
    [
        /<p class="timeline-group__summary">인터넷이 쌩쌩 달리면서 누구나 공짜로 쓰는 프로그램이 생겨났어요\. 마우스로 예쁜 그림을 콕콕 누르기만 하면 되는 쉬운 컴퓨터 세상이 활짝 열렸답니다\.<\/p>/g,
        '<p class="timeline-group__summary">인터넷의 대중화와 함께 오픈소스 생태계가 성장했습니다. 마우스로 아이콘을 클릭하는 직관적인 그래픽 사용자 인터페이스(GUI)가 표준으로 자리 잡았습니다.</p>'
    ],
    [
        /<p class="timeline-group__summary">필요할 때만 구름 너머의 컴퓨터를 빌려 쓰는 신기한 클라우드 세상이 열렸어요\. 게다가 주머니 속에 쏙 들어가는 똑똑한 전화기까지 등장했답니다\.<\/p>/g,
        '<p class="timeline-group__summary">필요한 만큼만 컴퓨팅 자원을 빌려 쓰는 클라우드 서비스가 등장했습니다. 또한, 손안의 컴퓨터인 스마트폰이 출시되며 모바일 시대가 본격화되었습니다.</p>'
    ],
    [
        /<p class="timeline-group__summary">정보가 산더미처럼 쌓이면서, 컴퓨터가 스스로 공부하는 인공지능이 쑥쑥 자라났어요\. 프로그램을 상자에 담아 어디든 쉽게 옮기는 마법 같은 기술도 생겨났죠\.<\/p>/g,
        '<p class="timeline-group__summary">방대한 데이터와 향상된 연산 능력을 바탕으로 딥러닝 기술이 비약적으로 발전했습니다. 소프트웨어를 쉽게 배포하고 관리할 수 있는 컨테이너 기술도 도입되었습니다.</p>'
    ],
    [
        /<p class="timeline-group__summary">모든 능력을 하나로 합친 슈퍼 칩이 노트북을 엄청나게 빠르게 만들었어요\. 게다가 사람처럼 글을 쓰고 그림을 그리는 인공지능 친구가 우리 곁에 찾아왔답니다\.<\/p>/g,
    ...
    [
        /<div class="event-detail__notes">\s*<p>배비지 할아버지가 생각한 '차분기관'은 어려운 수학 문제를 쉬운 덧셈으로 쪼개서 기계가 반복하게 만드는 마법 같은 설계였어요\. 금속 막대와 바퀴의 움직임으로 숫자를 읽어내는 아주 똑똑한 아이디어였죠\. 이 멋진 생각 덕분에 훗날 진짜 컴퓨터가 태어날 수 있었답니다\.<\/p>\s*<\/div>/g,
    ...
    [
        /<div class="event-detail__notes">\s*<p>아이다 아주머니는 구멍이 뽕뽕 뚫린 카드로 기계에게 할 일을 알려주는 방법을 생각했어요\. 계산하는 곳과 기억하는 곳을 나누고, "이럴 땐 이렇게 해!"라는 조건을 달아주는 똑똑한 방법도 글로 남겼죠\. 기계가 숫자 말고도 여러 가지 기호를 다룰 수 있다는 걸 처음으로 알아낸 대단한 발견이었어요\.<\/p>\s*<\/div>/g,
    ...
    [
        /<div class="event-detail__notes">\s*<p>튜링 아저씨가 상상한 기계는 할 일 목록, 지금 상태, 그리고 기호가 적힌 긴 테이프로 이루어져 있어요\. 기계의 눈과 손이 테이프를 한 칸씩 움직이며 기호를 읽고 쓰는 방식이죠\. 이 덕분에 아무리 복잡한 계산도 아주 작은 조각으로 나누어 생각할 수 있게 되었고, 컴퓨터가 할 수 있는 일과 없는 일을 분명하게 알게 되었어요\.<\/p>\s*<\/div>/g,
    ...
    [
        /<div class="event-detail__notes">\s*<p>이 기계는 0과 1만 쓰는 이진수, 전기로 껐다 켜는 스위치, 그리고 스스로 계산하는 능력을 하나로 합친 아주 놀라운 발명품이었어요\. 전기를 담아두는 통에 숫자를 기억하고, 결과는 구멍 뚫린 카드에 쏙쏙 남겼죠\. 이 멋진 실험 덕분에 더 크고 빠른 컴퓨터들이 태어날 수 있었어요\.<\/p>\s*<\/div>/g,
    ...
    [
        /<div class="event-detail__notes">\s*<p>에니악은 엄청나게 빠르고 똑똑했지만, 다른 계산을 하려면 사람들이 직접 수많은 전깃줄을 뽑았다가 다시 꽂아야 했어요\. 마치 전화 교환원처럼 말이죠\. 그래도 어떤 문제든 풀 수 있는 \'만능 기계\'였다는 점에서 컴퓨터 역사에 아주 큰 발자국을 남겼답니다\.<\/p>\s*<\/div>/g,
    ...
    [
        /<div class="event-detail__notes">\s*<p>트랜지스터는 전기가 흐르는 길을 열어주거나 막아주는 아주 작은 문지기 요정이에요\. 이 요정들이 모여서 0과 1이라는 컴퓨터의 언어를 만들어내죠\. 진공관 시대가 끝나고 트랜지스터 시대가 열리면서, 컴퓨터는 더 빠르고 똑똑하게 쑥쑥 자라날 수 있었어요\.<\/p>\s*<\/div>/g,
    ...
    [
        /<div class="event-detail__notes">\s*<p>이때부터 기계\(하드웨어\)와 프로그램\(소프트웨어\)을 따로따로 생각하기 시작했어요\. 그리고 여러 가지 일을 한꺼번에 척척 해내는 \'운영체제\(OS\)\'라는 똑똑한 지휘자도 본격적으로 활약하기 시작했답니다\. 컴퓨터가 진짜 일꾼으로 변신한 중요한 순간이었어요\.<\/p>\s*<\/div>/g,
    ...
    [
        /<div class="event-detail__notes">\s*<p>마이크로프로세서는 컴퓨터의 생각하는 뇌\(CPU\)를 아주 작게 줄여놓은 거예요\. 이 작은 뇌에 어떤 명령을 내리느냐에 따라 계산기도 되고, 신호등도 되고, 게임기도 될 수 있었죠\. 작고 똑똑한 칩 하나가 세상을 완전히 바꿔놓은 셈이에요\.<\/p>\s*<\/div>/g,
    ...
    [
        /<div class="event-detail__notes">\s*<p>IBM PC는 \'개방형 설계\'라는 멋진 아이디어를 썼어요\. 누구나 부품을 만들어서 끼워 넣을 수 있게 허락해 준 거죠\. 덕분에 똑똑한 사람들이 모여서 더 좋은 부품과 재미있는 프로그램을 잔뜩 만들었고, 지금 우리가 쓰는 컴퓨터의 튼튼한 뼈대가 되었어요\.<\/p>\s*<\/div>/g,
    ...
    [
        /<div class="event-detail__notes">\s*<p>웹은 HTML이라는 언어로 문서를 예쁘게 꾸미고, HTTP라는 규칙으로 문서를 주고받아요\. 팀 버너스리 아저씨는 이 엄청난 발명품을 "누구나 공짜로 쓰세요!"라며 세상에 선물했어요\. 이 따뜻한 마음 덕분에 인터넷은 모두의 놀이터이자 도서관이 될 수 있었어요\.<\/p>\s*<\/div>/g,
    ...
    [
        /<div class="event-detail__notes">\s*<p>리눅스는 \'오픈소스\'라는 멋진 문화를 활짝 꽃피웠어요\. 설계도를 꽁꽁 숨기지 않고 모두와 나누면, 더 빨리 발전하고 더 좋은 것을 만들 수 있다는 걸 증명했죠\. 지금 우리가 쓰는 스마트폰\(안드로이드\)이나 커다란 인터넷 서버들도 대부분 이 펭귄 요정\(리눅스\)의 도움을 받고 있어요\.<\/p>\s*<\/div>/g,
    ...
    [
        /<p class="event-detail__summary">"까만 화면에 하얀 글씨만 치는 건 너무 어려워! 마우스로 예쁜 그림을 콕콕 누르면 안 될까\?"라는 불평이 컴퓨터의 얼굴을 예쁘게 바꿨어요\.<\/p>/g,
        '<p class="event-detail__summary">"복잡한 텍스트 명령어 대신, 직관적인 그래픽으로 컴퓨터를 조작할 수 없을까?"라는 요구가 Windows 95의 대성공으로 이어지며 GUI 시대를 열었습니다.</p>'
    ],
    [
        /<div class="event-detail__story">\s*<img src="assets\/images\/story_computers_1995\.png" alt="1995년의 이야기 그림" class="story-image" style="width:100%; border-radius:8px; margin-bottom:1rem;">\s*<p>옛날에는 컴퓨터에게 일을 시키려면 어려운 영어 명령어를 키보드로 타닥타닥 쳐야 했어요\. 사람들은 "컴퓨터는 너무 어려워!"라며 도망갔죠\. 그래서 마이크로소프트 아저씨들은 \'윈도우 95\'라는 아주 쉽고 예쁜 화면을 만들었어요\.</p>\s*<p>이제 화면에 있는 예쁜 그림\(아이콘\)을 마우스로 콕콕 누르기만 하면 프로그램이 실행되었어요\. "시작" 버튼 하나로 모든 걸 할 수 있게 되자, 할아버지도 꼬마도 누구나 쉽게 컴퓨터와 친구가 될 수 있었답니다\.<\/p>\s*<\/div>/g,
    ...
    [
        /<div class="event-detail__notes">\s*<p>윈도우 95는 컴퓨터와 사람이 대화하는 방법\(인터페이스\)을 완전히 바꿔놓았어요\. 그림과 마우스를 쓰는 이 쉬운 방법 덕분에 전 세계 수많은 집과 학교에 컴퓨터가 놓이게 되었고, 인터넷 세상으로 들어가는 튼튼한 문이 활짝 열렸어요\.<\/p>\s*<\/div>/g,
    ...
    [
        /<div class="event-detail__notes">\s*<p>클라우드 덕분에 넷플릭스나 배달의민족 같은 멋진 서비스들이 쉽게 태어날 수 있었어요\. 손님이 갑자기 수백만 명으로 늘어나도, 클릭 몇 번이면 구름 너머의 컴퓨터를 순식간에 늘려서 척척 주문을 받을 수 있거든요\. 컴퓨터를 \'사는 물건\'에서 \'빌려 쓰는 전기\'처럼 바꿔놓은 엄청난 사건이에요\.<\/p>\s*<\/div>/g,
    ...
    [
        /<p class="event-detail__summary">"전화기, 카메라, MP3 플레이어, 인터넷을 하나로 합칠 수는 없을까\?"라는 스티브 잡스 아저씨의 마법 같은 주문이 세상을 깜짝 놀라게 했어요\.<\/p>/g,
        '<p class="event-detail__summary">"전화기, 미디어 플레이어, 인터넷 통신 기기를 하나의 직관적인 터치 기기로 통합할 수 없을까?"라는 비전이 스마트폰 시대를 열었습니다.</p>'
    ],
    [
        /<div class="event-detail__story">\s*<img src="assets\/images\/story_computers_2007\.png" alt="2007년의 이야기 그림" class="story-image" style="width:100%; border-radius:8px; margin-bottom:1rem;">\s*<p>애플의 스티브 잡스 아저씨는 무대 위에서 "오늘 우리는 세 가지 마법의 물건을 발표합니다\. 터치로 움직이는 아이팟, 새로운 전화기, 그리고 인터넷 통신 기기입니다\."라고 말했어요\. 사람들은 어리둥절했죠\. "세 개를 다 들고 다니라고\?"<\/p>\s*<p>하지만 잡스 아저씨는 씩 웃으며 말했어요\. "아니요, 이건 세 개가 아니라 하나의 기계입니다\. 바로 아이폰입니다!" 복잡한 키보드를 없애고 손가락으로 쓱쓱 넘기는 예쁜 화면을 가진 이 똑똑한 전화기는, 우리 주머니 속에 진짜 컴퓨터를 쏙 넣어준 역사적인 발명품이 되었어요\.<\/p>\s*<\/div>/g,
    ...
    [
        /<div class="event-detail__notes">\s*<p>아이폰은 '내 손안의 작은 컴퓨터' 시대를 활짝 열었어요\. 특히 누구나 자기가 만든 프로그램을 팔 수 있는 '앱스토어'라는 장터는, 기계를 만드는 회사뿐만 아니라 아이디어가 반짝이는 모든 사람에게 기회를 주어 세상을 크게 변화시키는 마법의 씨앗이 되었어요\.<\/p>\s*<\/div>/g,
    ...
    [
        /<div class="event-detail__notes">\s*<p>CUDA는 그림을 그리던 그래픽 카드를 '수학 천재'로 변신시켜 주는 마법의 지팡이예요\. 쉬운 계산을 한꺼번에 엄청나게 많이 해내는 요정들의 능력을 빌려서, 오늘날 컴퓨터가 스스로 공부하는 딥러닝과 인공지능 기술이 쑥쑥 자라날 수 있는 튼튼한 밭을 일구었답니다\.<\/p>\s*<\/div>/g,
    ...
    [
        /<div class="event-detail__notes">\s*<p>딥러닝은 사람의 머릿속 뇌세포들이 서로 손을 잡고 있는 모습을 흉내 낸 인공지능 기술이에요\. 엄청나게 많은 사진을 보고 공부해야 해서 시간이 아주 오래 걸렸지만, 그래픽 카드\(GPU\)의 도움 덕분에 짧은 시간 안에 똑똑해질 수 있었고, 드디어 인공지능의 황금빛 시대가 활짝 열렸어요\.<\/p>\s*<\/div>/g,
    ...
    [
        /<div class="event-detail__notes">\s*<p>컨테이너 기술은 프로그램을 포장하고 배달하는 마법 같은 방법이에요\. 어떤 컴퓨터에 가든 똑같은 모습으로 씩씩하게 일할 수 있도록 지켜주기 때문에, 수많은 사람이 모이는 커다란 인터넷 서비스를 흔들림 없이 튼튼하게 운영하는 데 꼭 필요한 보물 같은 기술이 되었어요\.<\/p>\s*<\/div>/g,
    ...
    [
        /<div class="event-detail__notes">\s*<p>Apple M1은 계산하는 뇌, 그림 그리는 뇌, 기억하는 방을 작은 칩 하나에 옹기종기 모아놓은 '통합 칩'이에요\. 친구들이 가까이 모여 있으니 심부름 갈 필요가 없어서 속도는 엄청 빠르고 전기는 아주 조금만 먹는답니다\. 이때부터 컴퓨터 마을 사람들은 "얼마나 전기를 아끼면서 일을 잘하느냐"를 가장 중요하게 생각하게 되었어요\.<\/p>\s*<\/div>/g,
    ...
    [
        /<p class="event-detail__summary">"이 거대한 기계들이 전부 똑똑한 로봇 친구 한 명을 가르치기 위한 거라고요\?" 사람들의 입이 떡 벌어지는 거대한 인공지능 학교가 세워졌어요\.<\/p>/g,
    ...
    [
        /<div class="event-detail__notes">\s*<p>스스로 글을 쓰고 그림을 그리는 생성형 AI는 상상할 수 없을 만큼 엄청난 계산 능력이 필요해요\. 그래서 수천 개의 그래픽 카드\(GPU\) 요정들을 빛보다 빠르게 연결하는 마법 같은 기술이 아주 중요해졌죠\. 이제 데이터를 모아두는 커다란 창고는 단순한 보관함을 넘어, 세상에서 가장 크고 똑똑한 거인 계산기로 쑥쑥 진화하고 있어요\.<\/p>\s*<\/div>/g,
