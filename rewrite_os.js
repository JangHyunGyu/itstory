...
html = html.replace(/<section class="timeline-group" aria-labelledby="decade-(\d{4})s? topic-\d{4}">\\s*<header class="timeline-group__head">\\s*<p class="timeline-group__decade" id="decade-\d{4}">(\d{4}s)<\/p>\\s*<h3 class="timeline-group__topic" id="topic-\d{4}">.*?<\/h3>\\s*<p class="timeline-group__summary">([\s\S]*?)<\/p>/g, (match, p1, p2, p3) => {
    if (summaries[p2]) {
        return match.replace(p3, summaries[p2]);
    }
    return match;
});
...
for (const year in events) {
    ...
    // Replace summary
    const summaryRegex = new RegExp((<template id="event--.*?">\\s*<article class="event-detail" aria-labelledby="event--.*?-title">\\s*<header class="event-detail__header">\\s*<p class="event-detail__year"><\/p>\\s*<h2 id="event--.*?-title">.*?<\/h2>\\s*<p class="event-detail__summary">)([\\s\\S]*?)(<\/p>));
    html = html.replace(summaryRegex, (match, p1, p2, p3) => {
        return p1 + event.summary + p3;
    });
    
    // Replace story
    const storyRegex = new RegExp((<template id="event--.*?">\\s*<article class="event-detail" aria-labelledby="event--.*?-title">\\s*<header class="event-detail__header">\\s*<p class="event-detail__year"><\/p>\\s*<h2 id="event--.*?-title">.*?<\/h2>\\s*<p class="event-detail__summary">.*?<\/p>\\s*<\/header>\\s*<section class="event-detail__body">\\s*<div class="event-detail__story">)([\\s\\S]*?)(<\/div>));
    html = html.replace(storyRegex, (match, p1, p2, p3) => {
        const imgTag = \\n\\t\\t\\t\\t\\t<img src="assets/images/story_os_.png" alt="년의 이야기 그림" class="story-image" style="width:100%; border-radius:8px; margin-bottom:1rem;">\\n\\t\\t\\t\\t\\t;
        return p1 + imgTag + event.story + '\\n\\t\\t\\t\\t' + p3;
    });
    
    // Replace notes
    const notesRegex = new RegExp((<template id="event--.*?">\\s*<article class="event-detail" aria-labelledby="event--.*?-title">\\s*<header class="event-detail__header">\\s*<p class="event-detail__year"><\/p>\\s*<h2 id="event--.*?-title">.*?<\/h2>\\s*<p class="event-detail__summary">.*?<\/p>\\s*<\/header>\\s*<section class="event-detail__body">\\s*<div class="event-detail__story">[\\s\\S]*?<\/div>\\s*<div class="event-detail__notes">\\s*)([\\s\\S]*?)(<\/div>));
    html = html.replace(notesRegex, (match, p1, p2, p3) => {
        return p1 + event.notes + '\\n\\t\\t\\t\\t' + p3;
    });
}
