...
$('.timeline-group__summary').each((i, el) => {
  if (i < timelineSummaries.length) {
    $(el).text(timelineSummaries[i]);
  }
});

// 2. Replace event-detail__summary
...
$('template').each((i, el) => {
  ...
  if (id && id.startsWith('event-')) {
    ...
    if (eventSummaries[year]) {
      $template('.event-detail__summary').text(eventSummaries[year]);
    }
    ...
    if (eventStories[year]) {
      ...
      $template('.event-detail__story').html(imgTag + '\\n' + eventStories[year]);
    }
    
    if (eventNotes[year]) {
      $template('.event-detail__notes').html(eventNotes[year]);
    }
