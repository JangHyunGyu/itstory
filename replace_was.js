...
let html = fs.readFileSync('c:/workspace/itstory/was.html', 'utf8');
...
if (!matches || matches.length !== replacements.length) {
    console.log(`Error: Found ${matches ? matches.length : 0} matches but have ${replacements.length} replacements.`);
} else {
    for (let i = 0; i < matches.length; i++) {
        html = html.replace(matches[i], replacements[i]);
    }
    fs.writeFileSync('c:/workspace/itstory/was.html', html, 'utf8');
