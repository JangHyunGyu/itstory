...
html_file = os.path.join('c:\\', 'workspace', 'itstory', 'os.html')
...
for decade, summary in summaries.items():
    decade_p = soup.find('p', class_='timeline-group__decade', string=decade)
    if decade_p:
        summary_p = decade_p.find_next_sibling('p', class_='timeline-group__summary')
        if summary_p:
            summary_p.string = summary

# Replace event details
for year, event in events.items():
    year_p = soup.find('p', class_='event-detail__year', string=year)
    if year_p:
        summary_p = year_p.find_next_sibling('p', class_='event-detail__summary')
        if summary_p:
            summary_p.string = event['summary']
        
        story_div = year_p.find_next('div', class_='event-detail__story')
        if story_div:
            story_div.clear()
            ...
            story_div.append(img_tag)
            story_soup = BeautifulSoup(event['story'], 'html.parser')
            for child in story_soup.children:
                story_div.append(child)
        
        notes_div = year_p.find_next('div', class_='event-detail__notes')
        if notes_div:
            notes_div.clear()
            notes_soup = BeautifulSoup(event['notes'], 'html.parser')
            for child in notes_soup.children:
                notes_div.append(child)
